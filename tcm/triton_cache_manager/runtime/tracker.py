...
from ..data.db_models import KernelOrm
from ..data.database import Database
from ..data.db_models import KernelOrm
...
class CacheAccessRecord:
    ...
    def __init__(self, cache_key: str, kernel_name: str, hit: bool, timestamp: float, cache_dir: Optional[Path] = None):
...
class RuntimeStatsCollector:
    ...
    def record_access(self, cache_key: str, kernel_name: str, hit: bool, cache_dir: Optional[Path] = None):
    ...
    def _create_stats_dict(self) -> StatsDict:
        ...
        try:
            ...
            try:
                for cache_key, stats in stats_by_key.items():
                    kernels = session.query(KernelOrm).filter(
                        KernelOrm.hash == cache_key,
                        KernelOrm.cache_dir == str(cache_dir)
                    ).all()
                    
                    for kernel in kernels:
                        current_hits: int = kernel.runtime_hits if kernel.runtime_hits is not None else 0
                        current_misses: int = kernel.runtime_misses if kernel.runtime_misses is not None else 0
                        
                        new_hits: int = stats["hits"]
                        new_misses: int = stats["misses"]
                        last_access: float = stats["last_access"]
                        
                        kernel.runtime_hits = current_hits + new_hits
                        kernel.runtime_misses = current_misses + new_misses
                        kernel.last_access_time = last_access
            ...
            log.error(f"Failed to persist runtime stats: {e}")
